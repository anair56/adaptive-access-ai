...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function init() {
    ...
    if (storage.settings) {
      ...
      aiSidebar.checked = storage.settings.showAISidebar !== false; // Default true
      updateSliderValues();
    }
    ...
    // Initialize Creator Hub buttons
    initCreatorHub();
  ...
  function updateGlobalMetrics(metrics) {
    if (metrics.totalAssists) {
      assistsValue.textContent = metrics.totalAssists;
    }
  }
  ...
  setInterval(async () => {
    if (currentTab) {
      try {
        const response = await chrome.tabs.sendMessage(currentTab.id, { type: 'GET_STATUS' });
        if (response) updateMetrics(response.metrics);
      } catch {}
    }
  }, 2000);
  ...
  // ------------------------------
  // ------------------------------
// Creator Hub Buttons (Theme-Matched)
// ------------------------------
